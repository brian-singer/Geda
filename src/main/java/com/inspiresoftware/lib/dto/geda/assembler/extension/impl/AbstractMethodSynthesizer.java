...
public abstract class AbstractMethodSynthesizer extends SynthesizerUtils implements MethodSynthesizer {
	...
	/**
	 * This method allows to "catch" generated classes by class generators such as Javassist or CGLib. The problem with
	 * those classes is that some of them are loaded by private ClassLoaders and are invisible to other generated
	 * classes such as DataReaders and DataWriters. The basic "catch" is to track $ sign in the class name that is put
	 * by both of those generators.
	 *
	...
	/**
	 * Default constructor synthesizers. This set a template for auto class generation. It is a requirement to provide a
	 * valid class loader to which files will be loaded. This in effect like a visitor pattern whereby we generate
	 * required file and enhance the current class loader by providing new class. The reason for this is that newly
	 * generated files need access to graph of objects available to class loader. Also it allows us to separate our
	 * contributions to each class loader.
	 *
	 * @param classLoader
	 *            class loader that will be used to load generated classes
	 */
	public AbstractMethodSynthesizer(final ClassLoader classLoader) {
		super();
		loader = initialiseClassLoaderWeakReference(classLoader);
	}

	/**
	 * Hook for sub classes to assign correct class loader reference. We must maintain only weak reference to the class
	 * loader.
	 *
	 * @param classLoader
	 *            class loader to contribute generated files to.
	 */
	...
	/**
	 * Hook for changing class loader reference. Invokes #initialiseClassLoaderWeakReference(getClassLoader()).
	 */
	protected final void enhanceClassLoader() {
		loader = initialiseClassLoaderWeakReference(getClassLoader());
	}
	 ...
	 *             any exceptions during configuration
	 */
	@Override
	...
	protected static class ReturnTypeContext {
		 ...
		 *            class of object that represent return type
		 * @param methodReturnTypePrimitiveName
		 *            primitive name (or null if this is not primitive)
		 */
		public ReturnTypeContext(final Class<?> clazz, final String methodReturnType,
	 ...
	 *            class name
	 * @param sourceClassGetterMethodReturnType
	 *            return type
	 * @return context
	 * @throws GeDARuntimeException
	 *             if unable to determine correct return type
	 */
	protected final ReturnTypeContext getReturnTypeContext(final String readerClassName,
		...
		try {
			...
			if (rcl.isPrimitive()) {
				return new ReturnTypeContext(rcl, PRIMITIVE_TO_WRAPPER.get(rcl.getCanonicalName()),
						rcl.getCanonicalName());
			}
			return new ReturnTypeContext(rcl, rcl.getCanonicalName(), null);
		} catch (GeDARuntimeException gre) {
			throw new GeDARuntimeException("Unable to determine correct return type from getter method in class: "
					+ readerClassName, gre);
		}
	}
	...
	public final DataReader synthesizeReader(final PropertyDescriptor descriptor)
		...
		if (reader == null) {
			...
			try {
				do {
					reader = makeReaderClass(getClassLoader(), readMethod, readerClassName, sourceClassNameFull,
							sourceClassGetterMethodName, readMethod.getGenericReturnType(), ctx);
					if (reader == null) {
						reader = getFromCacheOrCreateFromClassLoader(readerClassName, READER_CACHE, getClassLoader());
					} else {
	...
	private <T> T getFromCacheOrCreateFromClassLoader(final String className, final Cache<Object> cache,
			final ClassLoader classLoader) throws UnableToCreateInstanceException {
		Object instance;
	...
	/**
	 * Method to be overridden by specific synthesizer - contains the low level code to actually generating Class
	 * object.
	 *
	 * @param loader
	 *            class loader
	 ...
	 * @param sourceClassNameFull
	 *            name of the class of source object (i.e. whose getter will be invoked)
	 * @param sourceClassGetterMethodName
	 *            name of the getter method to be invoked on the source object
	 * @param sourceClassGetterMethodReturnType
	 *            class name of the return type to be returned
	 * @param ctx
	 *            compilation context. Need to invoke .next() for every unsuccessful compilation attempt.
	 *
	 * @return data reader instance.
	 *
	 * @throws UnableToCreateInstanceException
	 *             whenever there is a problem creating an instance of the generated class
	 * @throws GeDARuntimeException
	 *             any exceptions during class generation
	 */
	protected abstract DataReader makeReaderClass(final ClassLoader loader, final Method readMethod,
			final String readerClassName, final String sourceClassNameFull, final String sourceClassGetterMethodName,
	...
	protected class ArgumentTypeContext {
		 ...
		 * @param methodArgType
		 *            object class name
		 * @param methodArgPrimitiveName
		 *            primitive name (or null if this type is not primitive)
		 */
		public ArgumentTypeContext(final Class<?> clazz, final String methodArgType, final String methodArgPrimitiveName) {
			this.clazz = clazz;
	...
	/**
	 * @param sourceClassSetterMethodArgumentClass
	 *            class name of the argument type passed to setter
	 * @return context
	 */
	protected final ArgumentTypeContext getArgumentTypeContext(final Class<?> sourceClassSetterMethodArgumentClass) {
		...
		return new ArgumentTypeContext(sourceClassSetterMethodArgumentClass,
				sourceClassSetterMethodArgumentClass.getCanonicalName(), null);
	...
	public final DataWriter synthesizeWriter(final PropertyDescriptor descriptor)
		...
		if (writer == null) {
			...
			try {
				do {
					writer = makeWriterClass(getClassLoader(), writeMethod, writerClassName, classNameFull, methodName,
							writeMethod.getParameterTypes()[0], ctx);
					if (writer == null) {
						writer = getFromCacheOrCreateFromClassLoader(writerClassName, WRITER_CACHE, getClassLoader());
					} else {
	 ...
	 *            name of the reader class
	 * @param sourceClassNameFull
	 *            name of the class of source object (i.e. whose setter will be invoked)
	 * @param sourceClassSetterMethodName
	 *            name of the setter method to be invoked on the source object
	 * @param sourceClassSetterMethodArgumentClass
	 *            class name of the argument type passed to setter
	 * @param ctx
	 *            compilation context. Need to invoke .next() for every unsuccessful compilation attempt.
	 ...
	 *             whenever there is a problem creating an instance of the generated class
	 */
	protected abstract DataWriter makeWriterClass(final ClassLoader loader, final Method writeMethod,
			final String writerClassName, final String sourceClassNameFull, final String sourceClassSetterMethodName,
	...
	public static final class MakeContext {
		...
		/**
		 * To be used to attempt recovery through classloader instance creation. If next counter exceed the number of
		 * tries and exception is thrown, otherwise another cycle is attempted.
		 *
		 * @param exp
		 *            exception which occured whilst auto generating a class
		 * @param source
		 *            source code.
		 * @throws UnableToCreateInstanceException
		 *             thrown after number of tries is exceeded. wraps the original exception.
		 */
		public void next(final Exception exp, final String source) throws UnableToCreateInstanceException {
			tryNo++;
			if (tryNo > MAX_COMPILE_TRIES) {
				throw new UnableToCreateInstanceException(classType, "Unable to create class type [" + classType
						+ "]\n" + "with source:\n============>" + source + "\n<=============", exp);
			}
		}
	}
