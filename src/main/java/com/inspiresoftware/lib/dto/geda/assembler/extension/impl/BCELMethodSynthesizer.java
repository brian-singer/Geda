 ...
 * @since 1.1.0
 */
public class BCELMethodSynthesizer extends AbstractMethodSynthesizer implements MethodSynthesizer,
BaseDirectoryProvider {
	...
	private <T> T loadClass(final ClassLoader loader, final String className, final ClassGen cg) throws IOException,
	InstantiationException, IllegalAccessException {
		...
		final File clazz = new File(baseDir + readerSimpleName + ".class");
		clazz.deleteOnExit();
	...
	/** {@inheritDoc} */
	@Override
	protected DataReader makeReaderClass(final ClassLoader loader, final java.lang.reflect.Method readMethod,
			final String readerClassName, final String sourceClassNameFull, final String sourceClassGetterMethodName,
			final java.lang.reflect.Type sourceClassGetterMethodReturnType, final MakeContext ctx)
		...
		try {
			...
			final ClassGen cg = new ClassGen(readerClassName, "java.lang.Object", "<generated>",
					ACC_PUBLIC | ACC_SUPER, new String[] { DataReader.class.getCanonicalName() });
			...
			// read method
			final MethodGen read = new MethodGen(ACC_PUBLIC, org.apache.bcel.generic.Type.OBJECT,
					new org.apache.bcel.generic.Type[] { // argument types
					org.apache.bcel.generic.Type.OBJECT }, new String[] { "source" }, "read", readerClassName, il, cp);
			...
			il.append(factory.createInvoke(sourceClassNameFull, sourceClassGetterMethodName,
					rtc.isPrimitive() ? PRIMITIVE_TO_TYPE.get(rtc.getMethodReturnTypePrimitiveName()) : new ObjectType(
							enhanceInnerClassName(rtc.getMethodReturnType(), rtc.getClazz().isMemberClass())),
							org.apache.bcel.generic.Type.NO_ARGS,
							readMethod.getDeclaringClass().isInterface() ? Constants.INVOKEINTERFACE : Constants.INVOKEVIRTUAL));
			if (rtc.isPrimitive()) {
				il.append(factory.createInvoke(rtc.getMethodReturnType(), "valueOf",
						new ObjectType(rtc.getMethodReturnType()), new org.apache.bcel.generic.Type[] { // argument
					// types
					PRIMITIVE_TO_TYPE.get(rtc.getMethodReturnTypePrimitiveName()) }, Constants.INVOKESTATIC));
			}
			...
			// return type method
			final MethodGen returnType = new MethodGen(ACC_PUBLIC, org.apache.bcel.generic.Type.CLASS,
					org.apache.bcel.generic.Type.NO_ARGS, null, "getReturnType", readerClassName, il, cp);
			il.append(new LDC_W(cp.addClass(enhanceInnerClassName(rtc.getMethodReturnType(), rtc.getClazz()
					.isMemberClass()))));
			...
			throw new UnableToCreateInstanceException(readerClassName, "Unable to instantiate class: "
					+ readerClassName, ite);
		}
	}
	...
	/** {@inheritDoc} */
	@Override
	protected DataWriter makeWriterClass(final ClassLoader loader, final java.lang.reflect.Method writeMethod,
		...
		try {
			...
			final ClassGen cg = new ClassGen(writerClassName, "java.lang.Object", "<generated>",
					ACC_PUBLIC | ACC_SUPER, new String[] { DataWriter.class.getCanonicalName() });
			final ConstantPoolGen cp = cg.getConstantPool(); // cg creates constant pool
			cg.setMajor(MAJOR);
			cg.setMinor(MINOR);
			...
			// write method
			final MethodGen write = new MethodGen(ACC_PUBLIC, org.apache.bcel.generic.Type.VOID,
					new org.apache.bcel.generic.Type[] { // argument types
					org.apache.bcel.generic.Type.OBJECT, org.apache.bcel.generic.Type.OBJECT, }, new String[] {
					"source", "value" }, "write", writerClassName, il, cp);
			...
			if (atc.isPrimitive()) {
				...
				il.append(factory.createInvoke(atc.getMethodArgType(),
						...
						PRIMITIVE_TO_TYPE.get(atc.getMethodArgPrimitiveName()), org.apache.bcel.generic.Type.NO_ARGS,
			...
			il.append(factory.createInvoke(
					...
					new org.apache.bcel.generic.Type[] { atc.isPrimitive() ? PRIMITIVE_TO_TYPE.get(atc
							.getMethodArgPrimitiveName()) : new ObjectType(enhanceInnerClassName(
									atc.getMethodArgType(), atc.getClazz().isMemberClass())) }, writeMethod.getDeclaringClass()
									.isInterface() ? Constants.INVOKEINTERFACE : Constants.INVOKEVIRTUAL));
			...
			// arg type method
			final MethodGen paramType = new MethodGen(ACC_PUBLIC, org.apache.bcel.generic.Type.CLASS,
					org.apache.bcel.generic.Type.NO_ARGS, null, "getParameterType", writerClassName, il, cp);
			il.append(new LDC_W(cp.addClass(enhanceInnerClassName(atc.getMethodArgType(), atc.getClazz()
					.isMemberClass()))));
			il.append(InstructionFactory.ARETURN);
			...
			throw new UnableToCreateInstanceException(writerClassName, "Unable to instantiate class: "
					+ writerClassName, ite);
		}
	...
	/**
	 * @param configuration
	 *            configuration name baseDir - allows to set the directory where newly generated temp files for classes
	 *            will reside until the system exits. readerCleanUpCycle - allows to set clean up cycle for soft cache
	 *            of readers writerCleanUpCycle - allows to set clean up cycle for soft cache of writers
	 * @param value
	 *            value to set
	...
	public boolean configure(final String configuration, final Object value) throws GeDAException {
		if ("baseDir".equals(configuration) && value instanceof String) {
			...
			LOG.info("Setting class loader base dir to: {}", baseDir);
			enhanceClassLoader();
			return true;
		}
