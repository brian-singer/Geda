...
class MetadataChainAnnotationBuilder implements MetadataChainBuilder {
	...
	private List<PipeMetadata> buildVirtualFieldChain(final Field dtoField, final AnnotationProxy dtoFieldAnn) {
		...
		for (int index = 0; index < bindings.length; index++) {
			metas.add(new FieldPipeMetadata(dtoField.getName(), bindings[index], (String) dtoFieldAnn
					.getValue("dtoBeanKey"), getStringFromArray((String[]) dtoFieldAnn.getValue("entityBeanKeys"),
							index), (Boolean) dtoFieldAnn.getValue("readOnly"), (String) dtoFieldAnn.getValue("converter"),
							false, null, null));
		}
		return metas;
	}

	private List<PipeMetadata> buildFieldChain(final Field dtoField, final AnnotationProxy dtoFieldAnn,
			final AnnotationProxy dtoParentAnn) {

		final String[] bindings = createFieldBindingChain(getBindingFromAnnotationOrFieldName(
				(String) dtoFieldAnn.getValue("value"), dtoField.getName()));
		...
		for (int index = 0; index < bindings.length; index++) {
			metas.add(new FieldPipeMetadata(dtoField.getName(), bindings[index], (String) dtoFieldAnn
					.getValue("dtoBeanKey"), getStringFromArray((String[]) dtoFieldAnn.getValue("entityBeanKeys"),
							index), (Boolean) dtoFieldAnn.getValue("readOnly"), (String) dtoFieldAnn.getValue("converter"),
							dtoParentAnn != null, dtoParentAnn != null ? (String) dtoParentAnn.getValue("value") : null,
	...
	private List<PipeMetadata> buildCollectionChain(final Field dtoField, final AnnotationProxy dtoCollAnn)
		...
		final String[] bindings = createFieldBindingChain(getBindingFromAnnotationOrFieldName(
				(String) dtoCollAnn.getValue("value"), dtoField.getName()));

		final List<PipeMetadata> metas = new ArrayList<PipeMetadata>(bindings.length);
		for (int index = 0; index < bindings.length; index++) {
			metas.add(new CollectionPipeMetadata(dtoField.getName(), bindings[index], (String) dtoCollAnn
					.getValue("dtoBeanKey"),
					getStringFromArray((String[]) dtoCollAnn.getValue("entityBeanKeys"), index), (Boolean) dtoCollAnn
					.getValue("readOnly"), (Class) dtoCollAnn.getValue("dtoCollectionClass"),
					(String) dtoCollAnn.getValue("dtoCollectionClassKey"), (Class) dtoCollAnn
					.getValue("entityCollectionClass"), (String) dtoCollAnn
					.getValue("entityCollectionClassKey"), (Class) dtoCollAnn.getValue("entityGenericType"),
					(String) dtoCollAnn.getValue("entityGenericTypeKey"), (Class) dtoCollAnn
					.getValue("dtoToEntityMatcher"), (String) dtoCollAnn.getValue("dtoToEntityMatcherKey")));
		}
		return metas;
	}

	@SuppressWarnings("unchecked")
	private List<PipeMetadata> buildMapChain(final Field dtoField, final AnnotationProxy dtoMapAnn)
			throws UnableToCreateInstanceException {

		final String[] bindings = createFieldBindingChain(getBindingFromAnnotationOrFieldName(
				(String) dtoMapAnn.getValue("value"), dtoField.getName()));

		final List<PipeMetadata> metas = new ArrayList<PipeMetadata>(bindings.length);
		for (int index = 0; index < bindings.length; index++) {
			metas.add(new MapPipeMetadata(dtoField.getName(), bindings[index], (String) dtoMapAnn
					.getValue("dtoBeanKey"),
					getStringFromArray((String[]) dtoMapAnn.getValue("entityBeanKeys"), index), (Boolean) dtoMapAnn
					.getValue("readOnly"), (Class) dtoMapAnn.getValue("dtoMapClass"), (String) dtoMapAnn
					.getValue("dtoMapClassKey"), (Class) dtoMapAnn.getValue("entityMapOrCollectionClass"),
					(String) dtoMapAnn.getValue("entityMapOrCollectionClassKey"), (Class) dtoMapAnn
					.getValue("entityGenericType"), (String) dtoMapAnn.getValue("entityGenericTypeKey"),
					(String) dtoMapAnn.getValue("entityCollectionMapKey"), (Boolean) dtoMapAnn
					.getValue("useEntityMapKey"), (Class) dtoMapAnn.getValue("dtoToEntityMatcher"),
