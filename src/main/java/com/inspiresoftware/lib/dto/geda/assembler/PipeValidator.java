 ...
 * <p/>
 * User: Denis Pavlov Date: Jan 26, 2010 Time: 3:43:30 PM
 */
final class PipeValidator {
	 ...
	 * @param dtoWrite
	 *            method for writting data to DTO field
	 * @param dtoField
	 *            dto field
	 * @param entityRead
	 *            method for reading data from Entity field
	 * @param entityWrite
	 *            method for writting data to Entity field
	 * @param entityField
	 *            entity field
	 * @throws AnnotationMissingBindingException
	 *             if any of pipe is null (exception is thrown with a bit more clarification then the generic one).
	 */
	static void validatePipeNonNull(final DataReader dtoRead, final DataWriter dtoWrite, final String dtoField,
			final DataReader entityRead, final DataWriter entityWrite, final String entityField)
					throws AnnotationMissingBindingException {
	 ...
	 * @param entityRead
	 *            method for reading data from Entity field
	 * @param entityReadField
	 *            entity field
	 * @throws AnnotationMissingBindingException
	 *             if any of pipe is null (exception is thrown with a bit more clarification then the generic one).
	 */
	static void validateReadPipeNonNull(final DataWriter dtoWrite, final String dtoWriteField,
			final DataReader entityRead, final String entityReadField) throws AnnotationMissingBindingException {
	 ...
	 * @param entityWrite
	 *            method for writting data to Entity field
	 * @param entityWriteField
	 *            entity field
	 * @throws AnnotationMissingBindingException
	 *             if any of pipe is null (exception is thrown with a bit more clarification then the generic one).
	 */
	static void validateWritePipeNonNull(final DataReader dtoRead, final String dtoReadField,
			final DataWriter entityWrite, final String entityWriteField) throws AnnotationMissingBindingException {
	 ...
	 *            method to check
	 * @param type
	 *            type of reder/writer
	 * @param fieldName
	 *            field name
	 * @throws AnnotationMissingBindingException
	 *             if reader/writer is missing
	 */
	 ...
	 *             if arguments do not match (exception is thrown with a bit more clarification then the generic one).
	 */
	static void validatePipeTypes(final Registry registry, final DataReader dtoRead, final DataWriter dtoWrite,
			final String dtoField, final DataReader entityRead, final DataWriter entityWrite, final String entityField)
					throws AnnotationValidatingBindingException {
	 ...
	 *             if arguments do not match (exception is thrown with a bit more clarification then the generic one).
	 */
	static void validateReadPipeTypes(final Registry registry, final DataWriter dtoWrite, final String dtoField,
			final DataReader entityRead, final String entityField) throws AnnotationValidatingBindingException {
		...
		if (
			...
			throw new AnnotationValidatingBindingException(dtoField, dtoWrite.getClass().getCanonicalName(),
					dtoWriteClass.getSimpleName(), entityField, entityRead.getClass().getCanonicalName(),
					entityReadClass.getSimpleName(), false);
		}
	}
	 ...
	 *             if arguments do not match (exception is thrown with a bit more clarification then the generic one).
	 */
	static void validateWritePipeTypes(final Registry registry, final DataReader dtoRead, final String dtoField,
			final DataWriter entityWrite, final String entityField) throws AnnotationValidatingBindingException {
		...
		if (
			...
			throw new AnnotationValidatingBindingException(dtoField, dtoRead.getClass().getCanonicalName(),
					dtoReadClass.getSimpleName(), entityField, entityWrite.getClass().getCanonicalName(),
					entityWriteClass.getSimpleName(), true);
		}
