 ...
 * <p/>
 * User: Denis Pavlov Date: Jan 25, 2010 Time: 12:10:02 PM
 */
...
class MapPipe implements Pipe {
	 ...
	 * @param meta
	 *            collection pipe meta
	 *
	 * @throws AnnotationValidatingBindingException
	 *             when miss-mapped binding
	 */
	MapPipe(final AssemblerContext context, final DataReader dtoRead, final DataWriter dtoWrite,
			final DataReader entityRead, final DataWriter entityWrite, final MapPipeMetadata meta)
					throws AnnotationValidatingBindingException {
	...
	public void writeFromEntityToDto(final Object entity, final Object dto, final Map<String, Object> converters,
			final BeanFactory dtoBeanFactory) throws UnableToCreateInstanceException, BeanFactoryNotFoundException,
			BeanFactoryUnableToCreateInstanceException, CollectionEntityGenericReturnTypeException,
			AnnotationMissingException, InspectionScanningException, InspectionPropertyNotFoundException,
		...
		final Object entityCollection = entityRead.read(entity);

		if (entityCollection instanceof Collection) {

			final Collection entities = (Collection) entityCollection;

			final Map dtos = meta.newDtoMap(dtoBeanFactory);

			final Class entityRepresentative = meta.getReturnType(dtoBeanFactory);

			Object newDto = meta.newDtoBean(dtoBeanFactory);
			...
			try {
				...
				for (Object object : entities) {
					...
					newDto = meta.newDtoBean(dtoBeanFactory);
				}

				dtoWrite.write(dto, dtos);

			} catch (InspectionInvalidDtoInstanceException invDto) {
				throw new CollectionEntityGenericReturnTypeException(newDto.getClass().getCanonicalName(),
						meta.getDtoFieldName(), entityRepresentative != null ? entityRepresentative.getCanonicalName()
								: "unspecified");
			...
			final boolean useKey = meta.isEntityMapKey();
	...
	/** {@inheritDoc} */
	@Override
	public void writeFromDtoToEntity(final Object entityObj, final Object dto, final Map<String, Object> converters,
			final BeanFactory entityBeanFactory) throws BeanFactoryNotFoundException,
			BeanFactoryUnableToCreateInstanceException, UnableToCreateInstanceException,
			...
			ValueConverterNotFoundException, AnnotationMissingBeanKeyException, DtoToEntityMatcherNotFoundException,
		...
		final Object dtoColl = dtoRead.read(dto);
		...
		if (dtoColl instanceof Map) {
			...
			final Object originalEntityColl = entityRead.read(entity);

			Object original = null;
			if (originalEntityColl instanceof Collection || originalEntityColl instanceof Map) {
				original = originalEntityColl;
			} else {
				original = meta.newEntityMapOrCollection(entityBeanFactory);
				entityWrite.write(entity, original);
			}
			...
			final Object originalEntityColl = entityRead.read(entityObj);
	...
	private DataReader lazyCollectionKeyRead(DataReader reader, final Object dtoItem, final Object entityItem,
			final BeanFactory beanFactory) {
		if (reader == null) {
			if (meta.getMapKeyForCollection() != null && meta.getMapKeyForCollection().length() > 0) {

				Class representative = meta.getReturnType(beanFactory);
				...
				final PropertyDescriptor itemKeyDesc = PropertyInspector.getEntityPropertyDescriptorForField(
						dtoItem.getClass(), representative, meta.getDtoFieldName(), meta.getMapKeyForCollection(),
						itemDesc);
				reader = context.getMethodSynthesizer().synthesizeReader(itemKeyDesc);
			} else {
				throw new AnnotationValidatingBindingException(meta.getDtoFieldName(), dtoWrite.getClass()
						.getCanonicalName(), dtoWrite.getParameterType().getSimpleName(), meta.getEntityFieldName(),
						"", "", false);
			}
		}
		return reader;
	...
	private Assembler lazyCreateAssembler(final Assembler assembler, final Object dtoItem, final Object entityItem,
			final BeanFactory beanFactory) throws CollectionEntityGenericReturnTypeException,
			AnnotationMissingException, InspectionScanningException, UnableToCreateInstanceException,
			InspectionPropertyNotFoundException, InspectionBindingNotFoundException, AnnotationMissingBindingException,
			AnnotationValidatingBindingException, GeDARuntimeException, AnnotationDuplicateBindingException {
		if (assembler == null) {
			...
			try {
				if (Object.class.equals(representative)) {
					throw new CollectionEntityGenericReturnTypeException(dtoItem.getClass().getCanonicalName(),
							meta.getDtoFieldName(), representative.getCanonicalName());
				}
				return context.newAssembler(dtoItem.getClass(), representative);
			} catch (InspectionInvalidDtoInstanceException invDto) {
				throw new CollectionEntityGenericReturnTypeException(dtoItem.getClass().getCanonicalName(),
						meta.getDtoFieldName(), representative != null ? representative.getCanonicalName()
								: "unspecified");
			} catch (InspectionInvalidEntityInstanceException invEntity) {
				throw new CollectionEntityGenericReturnTypeException(dtoItem.getClass().getCanonicalName(),
						meta.getDtoFieldName(), representative != null ? representative.getCanonicalName()
	...
	private void addOrUpdateItems(final Object dto, final Map<String, Object> converters,
			final BeanFactory entityBeanFactory, final Collection original, final Map dtos)
					throws CollectionEntityGenericReturnTypeException, AnnotationMissingException,
					BeanFactoryNotFoundException, BeanFactoryUnableToCreateInstanceException, InspectionScanningException,
					...
					AnnotationDuplicateBindingException, InspectionInvalidDtoInstanceException,
		...
		Assembler assembler = null;
		final DtoToEntityMatcher matcher = meta.getDtoToEntityMatcher(converters);
		for (Object dtoKey : dtos.keySet()) {
			...
			if (toAdd) {
				final Object newItem = meta.newEntityBean(entityBeanFactory);
	...
	private void removeDeletedItems(final Map<String, Object> converters, final BeanFactory entityBeanFactory,
			final Collection original, final Map dtos) throws DtoToEntityMatcherNotFoundException,
			NotDtoToEntityMatcherException {
		final DtoToEntityMatcher matcher = meta.getDtoToEntityMatcher(converters);
	...
	private void removeDeletedItems(final Map<String, Object> converters, final BeanFactory entityBeanFactory,
			final Map original, final Map dtos) throws DtoToEntityMatcherNotFoundException,
			NotDtoToEntityMatcherException {
		final DtoToEntityMatcher matcher = meta.getDtoToEntityMatcher(converters);
		final List keysToRemove = new ArrayList(); // must save to avoid concurrent modification exception while
		// #remove(key)
