...
class BasePipeMetadata implements PipeMetadata {
	 ...
	 */
	public BasePipeMetadata(final String dtoFieldName, final String entityFieldName, final String dtoBeanKey,
			final String entityBeanKey, final boolean readOnly) {
	...
	/** {@inheritDoc} */
	@Override
	public Object newDtoBean(final BeanFactory factory) throws BeanFactoryNotFoundException,
	BeanFactoryUnableToCreateInstanceException {
		return newBean(getDtoBeanKey(), factory, true);
	}
	...
	/** {@inheritDoc} */
	@Override
	public Object newEntityBean(final BeanFactory factory) throws BeanFactoryNotFoundException,
	BeanFactoryUnableToCreateInstanceException {
		return newBean(getEntityBeanKey(), factory, false);
	}
	...
	protected Class getRepresentation(final String beanKey, final BeanFactory factory, final boolean isDto)
		...
		if (factory == null) {
			if (isDto) {
				throw new BeanFactoryNotFoundException(dtoFieldName, beanKey, true);
			} else {
				throw new BeanFactoryNotFoundException(entityFieldName, beanKey, false);
			}
		}
		final Class representation = factory.getClazz(beanKey);
		if (representation == null) {
			if (isDto) {
				throw new BeanFactoryUnableToLocateRepresentationException(factory.toString(), dtoFieldName, beanKey,
						true);
			} else {
				throw new BeanFactoryUnableToLocateRepresentationException(factory.toString(), entityFieldName,
						beanKey, false);
			}
		}
	 ...
	 *            bean factory
	 * @param isDto
	 *            true if this is a DTO instance
	 * @return new instance.
	 *
	 * @throws BeanFactoryNotFoundException
	 *             if bean factory is null
	 * @throws BeanFactoryUnableToCreateInstanceException
	 *             if bean factory returns null
	 */
	protected Object newBean(final String beanKey, final BeanFactory factory, final boolean isDto)
		...
		if (factory == null) {
			if (isDto) {
				throw new BeanFactoryNotFoundException(dtoFieldName, dtoBeanKey, true);
			} else {
				throw new BeanFactoryNotFoundException(entityFieldName, entityBeanKey, false);
			}
		}
		final Object newObject = factory.get(beanKey);
		if (newObject == null) {
			if (isDto) {
				throw new BeanFactoryUnableToCreateInstanceException(factory.toString(), dtoFieldName, dtoBeanKey, true);
			} else {
				throw new BeanFactoryUnableToCreateInstanceException(factory.toString(), entityFieldName,
						entityBeanKey, false);
			}
		}
		return newObject;
