...
/**
 * User: denispavlov Date: 12-09-20 Time: 1:56 PM
 */
public class DtoEntityContextByClass extends DSLUtils implements DtoEntityContext {
	...
	public DtoEntityContextByClass(final Class dtoClass, final Class entityClass,
	...
	public DtoEntityContext alias(final String beanKey, final Class representative) {
		...
		if (representative == null) {
			beanFactory.registerEntity(beanKey, entityClass.getCanonicalName(), entityClass.getCanonicalName());
		} else if (representative.isInterface() && representative.isAssignableFrom(entityClass)) {
			beanFactory.registerEntity(beanKey, entityClass.getCanonicalName(), representative.getCanonicalName());
		} else {
	...
	public DtoEntityContextAppender withFieldsSameAsIn(final String beanKey, final String... excluding) {
		if (beanFactory == null) {
			throw new GeDARuntimeException(
					"Bean for key "
							+ beanKey
							+ " cannot be looked up. Bean factory must be specified. Use constructor DefaultDSLRegistry(BeanFactory)");
		}
