...
/**
 * User: denispavlov Date: 13-05-24 Time: 8:50 AM
 */
public class ClassLoaderBeanFactory implements ExtensibleBeanFactory {
	 ...
	 *            simple key to class mapping
	 * @param mappingEntityRepresentatives
	 *            simple key to interface mapping for entities
	 */
	public ClassLoaderBeanFactory(final ClassLoader classLoader, final Map<String, String> mappingClasses,
		...
		mappingReps.putAll(mappingClasses);
		// override all entity ones
		mappingReps.putAll(mappingEntityRepresentatives);
	}
	...
	public void registerDto(final String key, final String className) throws IllegalArgumentException {
		if (key != null && key.length() > 0 && className != null && className.length() > 0) {
			if (mappingClasses.containsKey(key)) {
				throw new IllegalArgumentException("key is already used: [key=" + key + ", className=" + className
						+ "]");
			}
			mappingClasses.put(key, className);
			mappingReps.put(key, className);
		} else {
	...
	/** {@inheritDoc} */
	@Override
	public void registerEntity(final String key, final String className, final String representative)
			throws IllegalArgumentException {
		if (key != null && key.length() > 0 && className != null && className.length() > 0 && representative != null
			...
			mappingClasses.put(key, className);
			mappingReps.put(key, representative);
		} else {
			throw new IllegalArgumentException("all args are mandatory: [key=" + key + ", className=" + className
					+ ", rep=" + representative + "]");
		}
	...
	private Class getClassFromMapping(final String entityBeanKey, final Map<String, String> mapping,
