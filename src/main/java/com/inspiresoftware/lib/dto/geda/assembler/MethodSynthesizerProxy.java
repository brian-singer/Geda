...
class MethodSynthesizerProxy implements MethodSynthesizer {
	...
	static {
		FACTORY.put("javassist", "com.inspiresoftware.lib.dto.geda.assembler.extension.impl.JavassistMethodSynthesizer");
	 ...
	 *             when instance cannot be configured
	 */
	public MethodSynthesizerProxy(final ClassLoader classLoader, final Object value)
			throws UnableToCreateInstanceException {
		this(classLoader);
	...
	private MethodSynthesizer lazyGet(final ClassLoader classLoader, final Object value)
			throws UnableToCreateInstanceException {
		if (synthesizer == null) {
			try {
				lock.lock();
				if (synthesizer == null) {
					if (value == null) {
						// default impl
						final Class clazz = Class.forName(synthesizerImpl);
						synthesizer = (MethodSynthesizer) clazz.getConstructor(ClassLoader.class).newInstance(
								classLoader);

					} else {
						if (value instanceof MethodSynthesizer) {
							synthesizer = (MethodSynthesizer) value;
							synthesizerImpl = value.getClass().getCanonicalName();
						} else if (value instanceof String) {
							final String newImpl = FACTORY.get(value);
							if (newImpl != null) {
								synthesizerImpl = newImpl;
							} else {
								synthesizerImpl = value.toString();
							}
							final Class clazz = Class.forName(synthesizerImpl);
							synthesizer = (MethodSynthesizer) clazz.getConstructor(ClassLoader.class).newInstance(
									classLoader);
						} else {
							throw new UnableToCreateInstanceException("MethodSynthesizer", "Unable to create [" + value
									+ "] implementation: " + "configuration not recognizer (Must be either: "
				...
				throw new UnableToCreateInstanceException("MethodSynthesizer", "Unable to create [" + synthesizerImpl
						+ "] implementation: " + exp.getMessage(), exp);
			} finally {
				lock.unlock();
			}
			if (clRef != null) {
				clRef.clear(); // release class loader as soon as we do not need it anymore.
			}
		} else if (value != null) {
			LOG.warn("Synthesizer is already set to: {}, configuration [{}] ignored", synthesizer.getClass()
